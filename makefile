...
test: all-class config-test
	java src.MS2Frame sample-input/*.`head -1 config/mask-suffix.head-1` | ffmpeg -r `head -1 config/r.head-1` -i pipe:0 -vcodec libx264 -pix_fmt yuv420p -acodec aac -y -ss 0 -t `head -1 config/t.head-1` "video/`date`.mp4"
...
sample: all-class config-sample
	java src.MS2Frame sample-input/*.`head -1 config/mask-suffix.head-1`
